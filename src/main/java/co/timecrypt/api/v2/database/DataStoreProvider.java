...
/**
 * This class should handle creation of {@link TimecryptDatabase}(s). If you need a more complex implementation, override that here.
 */
public class DataStoreProvider {

    private static Map<String, TimecryptDatabase> storeCache = new HashMap<>();
    ...
    public static void onServletInitialized(TimecryptApiServlet servlet) {
        TimecryptDatabase dataStore = new SimplePostgresqlDatabase();
        try {
     ...
     */
    public static TimecryptDatabase getDataStore(TimecryptApiServlet servlet) {
    ...
    public static void onServletDestroyed(TimecryptApiServlet servlet) {
        TimecryptDatabase dataStore = storeCache.get(servlet.getClass().getSimpleName());
        dataStore.destroy();
