...
class DestructDateFragment : TimecryptFragment(), CircularSlider.OnSliderMovedListener {
    ...
    private fun convertToDate(mapPick: Pair<Int, Int>): Date {
        val localTime = when (mapPick.second) {
            R.plurals.plural_day -> LocalDate.now().atStartOfDay().plusDays(mapPick.first.toLong())
            R.plurals.plural_week -> LocalDate.now().atStartOfDay().plusWeeks(mapPick.first.toLong())
            R.plurals.plural_month -> LocalDate.now().atStartOfDay().plusMonths(mapPick.first.toLong())
            R.plurals.plural_year -> LocalDate.now().atStartOfDay().plusYears(mapPick.first.toLong())
